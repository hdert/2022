...
def update_balance():
    ...
    for account in account_list:
        ...
        balance_string += "{}: ${:.2f} - {:.0f}% of ${:.2f} goal\n".format(
            account.name, account.balance, progress, account.goal)
...
def deposit_money(account):
    if account.deposit(amount.get()):
        ...
        action_feedback.set(
            "Success! Total of ${:.2f} deposited into {}".format(
                amount.get(), account.name))
...
def withdraw_money(account):
    if account.withdraw(amount.get()):
        ...
        message_text.set(message)
        action_feedback.set(
            "Success! Total of ${:.2f} withdrawn from {}".format(
                amount.get(), account.name))
        image_label.configure(image=sad_image)
    else:
        action_feedback.set(
            "Not enough money in {} or not a valid amount".format(
                account.name))
...
account_list = []
deposit_messages = [
    "Well done, keep those deposits coming!", "You're making good progress!",
    "Awesome! It will feel great when you reach your goal"
]
withdraw_messages = [
    "Think about where else you might be able to save some money this week",
